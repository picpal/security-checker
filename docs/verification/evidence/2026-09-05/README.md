...
1차 `a483b3b1-standard` 실행(04:09~04:35, `elapsed_s=1543.0`)은 trivy 가 900초 만에 타임아웃했다. (추정) 원인은 로컬 trivy DB(2026-08-06)가 오래되어 최초 스캔 시 trivy 가 자동으로 취약점 DB(대용량 Java DB 포함)를 내려받으려 했기 때문이며, BOM 생성(cdxgen) 자체는 정상(15초 만에 196개 컴포넌트, mssql 13.2.1.jre11·tomcat 11.0.22 보존)이었다. (추정) 같은 실행에서 도달성도 `reason=error` 로 실패했는데, 이는 depscan 이 엔진 예산 내에 사용처(usage) 슬라이스 산출을 마치지 못한 것으로, 엔진이 예외를 삼키고 `reason` 만 남기는 동작이다(백로그 후보로 별도 기록 필요).
