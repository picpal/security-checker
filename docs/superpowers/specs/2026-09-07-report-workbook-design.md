...
**1_취약점 목록** (17열, 우선순위 → 심각도 → 위치 정렬, 틀고정 D2)
...
**3_판정기준 → 3_용어와 대응(정오표 (h))** (고정 표, 골든): 판정 규칙 · 우선순위 규칙 · 담당 규칙 · 심각도 뜻 · 도달성 주의 · 사실/추정 구분 · 자동 억제 없음 · 조치 유형 enum 뜻.
...
- (h) 2026-09-08 시트 이름 `3_판정기준` → `3_용어와 대응`: 내용이 규칙 판정이 아니라 용어의 뜻과 대응 방법이라 이름을 맞춤(사용자 결정).
- (i) 2026-09-08 시트 이름 `1_조치목록` → `1_취약점 목록`: '조치 목록' 은 이미 처리한 목록처럼 읽힘(사용자 결정). 본문의 `1_취약점 목록` 표기는 이 개명을 소급 적용한 것.
