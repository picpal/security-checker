...
(f) 보고서·request·결과반환의 finding ID 는 --target 상대화 공간 기준이다. --check-result 는
    ...
    우선).
- (g) 2026-09-07 가독성 개선: `3_판정기준` 은 4열(구분·값·용어·뜻·그래서 어떻게 하나) 일반어 표로, `4_결과반환` 은 한국어 열 이름 + 상단 안내 6행(첫 열이 12-hex ID 가 아니면 안내 행) + 참고 열 3개(문제·담당·위치, 검증기는 무시). 워크북에 열 너비·줄바꿈·굵은 헤더·틀고정 서식을 넣되 modified 는 created 로 재고정(결정성 유지).
