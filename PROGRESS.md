...
2. **코드가 만드는 좌표** — `"$g:$a:$v"`(좌표 전체가 변수), `implementation depString`, 반복문·
   조건문으로 조립하는 의존성. kts 에서 값이 코드로 오는 표기(`by project.provider { … }`,
   `System.getenv(...)`)도 같은 부류다. (Groovy 명명인자 표기는 2026-09-09 에 대응 완료. kotlin DSL 의
...
- [x] **파서 경계 4건(codex)**: (1) 여는 괄호 뒤 줄바꿈 이어쓰기 — 후행 콤마만 보던 것을
  **괄호·대괄호 깊이 추적**으로 교체, (2) `version:` 선행 맵·대괄호 맵 — **호출을 먼저 인식하고
  필수 키 충족은 파싱 후 검증**(FP0 근거인 세 키 요구는 그대로), (3) 한 줄에 `;` 로 두 호출 —
  깊이 0 세미콜론·개행을 문장 경계로, 호출 단위로 args 분리, (4) 삼중따옴표 예시 코드 —
  주석 제거만으로는 부족해 **문자열 리터럴을 자리표시자로 치환한 코드 골격**에서 파싱한다
  (값은 번호로 원문 복원). 호출 정규식은 식별자와 인자 사이에 여는 구분자나 공백을 강제한다
  — 없으면 `name:` 의 'nam'+'e:' 가 호출로 잡혀 인자가 쪼개진다.
- [x] **파서 경계 3건(codex 최종)**: (1) 중첩 호출 — `implementation(platform(group: …))` 처럼
  식별자 앞이 `(`/`[` 인 경우를 문자 클래스에서 빠뜨렸다(주석과 코드가 어긋나 있었다). 문자열은
  이미 골격에서 자리표시자라 앞 문자 제한이 문자열 방어를 겸하지 않으므로 넓혀도 안전하다.
  (2) **명명인자 파서를 Groovy(`.gradle`) 전용으로 한정** — kts 에서 콜론은 타입 주석이라
  `fun dep(group: String, name: String, version: String)` 이 의존성으로 잡혀 **kts 프로젝트가 매
  스캔 캐시를 잃었다**(kotlin 명명인자는 `=` 라 애초에 미대응). 회귀 픽스처 `fixtures/kts-app`
  추가 + FP0 게이트 테스트. (3) Groovy slashy(`/…/`)·dollar-slashy(`$/…/$`) 문자열을 렉서가
  인식한다. slashy 는 나눗셈과 정적으로 구분 불가라 **값 자리(`= ( [ { , :` 뒤)에서 실제로
  닫힐 때만** 문자열로 본다. 닫히지 않은 따옴표가 **개행을 삼키던 버그**도 함께 고쳤다(다음 줄의
  진짜 선언이 가려졌다).
- [x] kts 회귀 픽스처를 넣고 나서 **측정에서 드러난 추가 오탐 1건**도 고쳤다 — kts 표준 표기
  `val x: String by extra("1.12.0")` / `extra["x"] = "…"` 를 심볼표가 못 읽어 kts 멀티모듈이 매 스캔
  우회했다. FP0 실측을 **22개 대상**(kts 3종 포함)으로 넓혔다.
