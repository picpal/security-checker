...
## 상태: M0~M5 + post-M5 + D(커스텀 룰셋) 완료 ✅ (433 passed, 2 xfailed, 2026-09-06) · V 플랜2 완료(판정 H·xlsx·gate 생성기; 최종 측정 docs/measurements/2026-09-06-message-gate-verification-final.md) · 보고서 워크북(secscan report, 5 시트, 해석 파일 검증기, --check-result) 완료
<!-- goal 의 모든 마일스톤·전체 완료 기준 충족. 이후 도그푸딩/실프로젝트 기반 강화. -->
- **post-M5**: detect 노이즈제외 / exclude(기본제외+`.gitignore`) / **BOM-SCA**(cdxgen→trivy sbom, 락파일없는 gradle SCA 0→34; BOM 캐시 키=대상경로+매니페스트 내용 해시 — 2026-09-09 스테일 BOM 수정; 동적버전/SNAPSHOT/원격parent 는 dynamic_versions 로 감지해 자동 캐시우회(좌표문자열+groovy 명명인자 both), `--no-bom-cache` 수동 우회; 지문·탐지 범위=collect_manifests(대상+로컬 parent 체인+gradle 빌드루트), 버전 표현은 해석 실패 시 fail-closed=동적; 불확실 신호 5종(dynamic/unresolved/ambiguous/unparsable/external-script)→우회 + BOM TTL 기본 24h `--bom-max-age`. 정적 분석 원리적 한계 8종은 PROGRESS — gradle 심볼 가시성은 의도적 미수정(오탐 폭발 회피), TTL 이 백스톱) / SARIF locations / **컴플라이언스 매핑**(compliance.py: CWE→KISA49/PCI6.2.4) / **SAST 확대(B)**: Semgrep 팩 확대 + confidence 계층화(sast_tier actionable/review, missing→review) + severity 한글 + exit-code 게이팅(review 비차단) + merge 보수적 + doctor 런타임점검. / **D(커스텀 룰셋)**: semgrep 룰 3종(secscan/rules/: MyBatis ${} value=actionable·identifier=review 위치분기 / 하드코딩 자격증명 / zero-salt) + `--config` 통합(코어무변경) + is_test_path 경로강등(test/loadtest→review, 전 SAST) + 외부repo비의존 픽스처(fixtures/custom-rule-app) 회귀게이트(양성+음성 FP0). d4a1ded 취약시점 대조로 갭 3종 실증(gitleaks/semgrep 미탐→커스텀룰 탐지). **C(함수간 taint) 불필요 확정** — spotbugs 실행돼도 CWE-476/754/404 미탐(IDE 인스펙션 범주, taint 아님). docs/measurements/2026-06-28-gap-map.md.
...
- **아키텍처**: 새 스캐너 = adapters/<tool>.py + normalize/<tool>.py + normalize._PARSERS 한 줄 + profiles + doctor REQUIREMENTS. 카테고리 전용(도달성=SCA, 검증=secret). 실제 스캔(네트워크/빌드)은 Bash dangerouslyDisableSandbox=true. Finding.dedup_key가 억제 매칭 키. **커스텀 룰 추가 = secscan/rules/<rule>.yml(semgrep; metadata.cwe/confidence 필수 → tier/compliance 자동) + fixtures/custom-rule-app 픽스처 + 골든.**
...
- **백로그(미착수)**: IaC/DAST/CI/주기점검/Dependency-Track/IDE 인스펙션 통합(broad-catch 등 코드스멜 = coderay 잔여격차)/LSP. (Opengrep/CodeQL 함수간 taint = 갭 지도로 **불필요 확정**.)
